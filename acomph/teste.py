...
def excel_to_dataframe(dict_geral, str_string):  # (Dicionario de bacias, nome da bacia)
    df = pd.read_excel(r'C:\Users\Lucas\Downloads\ACOMPH_19.11.2019.xls', str_string, skiprows=4, nrows=30,
...
def fix_dataframe(dataframe, bacia):
    if bacia == 'Grande':
        ...
        aux = pd.read_excel(r'C:\Users\Lucas\Downloads\ACOMPH_11.11.2019.xls', 'Tietê', skiprows=4,
...
def outros(dataframe):
    aux1 = [dataframe[237], dataframe[238], dataframe[239], dataframe[240], dataframe[242],
            dataframe[243], dataframe[244], dataframe[245], dataframe[246], dataframe[266]]
    aux2 = [37, 38, 39, 40, 42, 43, 44, 45, 46, 66]
...
def main():
    ...
    teste[292] = teste['correcao']
    teste[302] = teste[288] - teste['correcao']
    # a = 0
    # for i in teste.data:
    ...
    for i in range(len(teste.data)):
        teste.data[i] = teste.data[i].date()
    print(teste.data)
    # print(teste[288])
    print(dir(mysql))
    # cnx = mysql.connector.connect(user='Lucas', password='Vodka123', host='127.0.0.1', database='urca')
    # cursor = cnx.cursor()
    # cols = "`,`".join([str(i) for i in teste.columns.tolist()])
        # for i, row in teste.iterrows():
        #    sql = "INSERT INTO `book_details` (`" + cols + "`) VALUES (" + "%s," * (len(row) - 1) + "%s)"
    #    cursor.execute(sql, tuple(row))

    # engine = create_engine(
    #    'mysql+mysqlconnector://' + os.environ['Lucas'] + ':' + os.environ['Vodka123'] + '@' + os.environ[
    #       '127.0.0.1'] + ':' + os.environ['3306'] + '/sandbox', echo=False)
    teste = round(teste, 3)
    ...
    teste.to_sql(con=engine, name='vazao', if_exists='replace', index=False)
